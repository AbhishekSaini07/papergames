...
import { setFirebaseData, setPlayerInGame, setPlayerSelectedNum, setWinInGame } from "../../services/bingo_firebase_services";
...
const BingoBoard1 = () => {
  ...
  const [play1Num, setNum] = useState(0);
  ...
  const handleUpdateTurnField = async () => {
    ...
    try{
      setFirebaseData(gameCode,"turn",2);
    }
    catch(error){
  ...
  const handleUpdatePlay2NumField = async (num) => {
    try{
        setFirebaseData(gameCode,"play2Num",num);
      }
      catch(error){
        console.log(error.message);
      }
  ...
  const handleClick = (index) => {
    handleUpdatePlay2NumField(arr[index]);
    setStrickoutCells((prevCells) => {
      const newCells = [...prevCells];
      newCells[index] = true;
      return newCells;
  ...
  return (
    <div className="nm">
      ...
      <Container fluid>
        ...
        <Row>
          <Col>
            <div className={style.bx1} disabled={1 != player}>
          ...
          <button onClick={handleCheck}>check</button>
            ...
            <Row>
              <Col>
                <div className={style.bx2}>
                  <h3>{1 == player ? "Your Turn" : "Second Player Turn"}</h3>
                </div>
              </Col>
            </Row>
            <Row>
